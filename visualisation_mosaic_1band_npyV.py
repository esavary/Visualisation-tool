...
class BoxLayoutMosaic(BoxLayout_main):
    ...
    def prepare_png(self, number):
        ...
        for i in np.arange(start, start + number + 1):
            ...
            plt.imsave(self.pathtoscratch + str(i + 1) + self.scale_state + self.colormap + str(start) + '.png', img,
                       cmap=self.colormap)
    ...
    def update(self, event):
        ...
        for button in self.list_of_buttons:
            ...
            try:
                ...
                button.set_background_normal(self.pathtoscratch + str(i + 1) + self.scale_state +self.colormap + str(start) + '.png')
    ...
    def on_click(self, number, event):
        if 100 * self.forward_backward_state + number > len(self.listimage):
            ...
            self.dataframe.to_csv('./classifications/classification_mosaic_autosave' + str(self.random_seed)+'.csv', index=False)
